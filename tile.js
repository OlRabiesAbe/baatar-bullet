class Tile extends Entity {
	constructor(game, img, imgX, imgY, x, y) {
		...
		this.VERT_LOWER_COLL_RADIUS = 32; //'' how far a tile's vertical hitboxes into the tile
		this.VERT_UPPER_COLL_RADIUS = 0; // how far a tile's vertical hitboxes out of the tile
	...
	handleEntityCollision(entity) {
		...
		if(entity.y < this.y - this.VERT_UPPER_COLL_RADIUS || entity.y - entity.height > this.y + this.height + this.VERT_UPPER_COLL_RADIUS) {}
			...
			//a tile's left/right colliders extends HCR units into and out of the tile
					// ~+LEFT CASE+~
			if(entity.x + (entity.width/2)+1 > this.x - this.HORZ_COLL_RADIUS && entity.x + (entity.width/2)+1 < this.x + this.HORZ_COLL_RADIUS 
							&& entity.y > this.y + this.ARB_HITBOX_BUFFER && entity.y - entity.height < this.y + this.height - this.ARB_HITBOX_BUFFER) {
				entity.x = this.x - this.HORZ_COLL_RADIUS - (entity.width/2)+1;
				entity.hspeed = 0;
			}
					// ~+RIGHT CASE+~
			if(entity.x - (entity.width/2)+1 < this.x + this.width + this.HORZ_COLL_RADIUS && entity.x - (entity.width/2)+1 > this.x + this.width - this.HORZ_COLL_RADIUS 
							&& entity.y > this.y + this.ARB_HITBOX_BUFFER && entity.y - entity.height < this.y + this.height - this.ARB_HITBOX_BUFFER) {
				entity.x = this.x + this.width + this.HORZ_COLL_RADIUS + (entity.width/2)+1;
				entity.hspeed = 0;
			}
					// ~+TOP CASE+~
			if(entity.y > this.y - this.VERT_UPPER_COLL_RADIUS && entity.y < this.y + this.VERT_LOWER_COLL_RADIUS 
				...
				entity.vspeed = 0;
					...
					// ~+BOTTOM CASE+~
			if(entity.y - entity.height < this.y + this.height + this.VERT_UPPER_COLL_RADIUS && entity.y - entity.height > this.y + this.height - this.VERT_LOWER_COLL_RADIUS 
							&& entity.x + (entity.width/2) > this.x + this.ARB_HITBOX_BUFFER && entity.x - (entity.width/2) < this.x + this.width - this.ARB_HITBOX_BUFFER) {
				entity.y = this.y + this.height + this.VERT_UPPER_COLL_RADIUS + entity.height;
				entity.vspeed = 0;
			}
		}
	}
